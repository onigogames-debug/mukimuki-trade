...
const stopTickers = new Set(['AI', 'EV', 'EST', 'JPY', 'USD', 'FAQ', 'PR']);
...
export const buildArticleIndex = ({ articles = [], posts = [], performanceReports = [] } = {}) => {
  ...
  for (const article of [...generatedArticles, ...postArticles, ...performanceArticles]) {
    if (!article.path || merged.has(article.path)) continue;
...
export const renderRelatedArticlesSection = (current, index, { escapeHtml, max = 3 } = {}) => {
  ...
  return `      <section class="article-panel related-articles" aria-labelledby="related-articles-title">
        ...
        <div class="link-grid">
${related.map((article) => `          <a class="link-card" href="${escape(article.path)}"><span>${escape(article.category || '関連記事')}</span><strong>${escape(article.title)}</strong><p>${escape(article.description || article.summary || '')}</p></a>`).join('\n')}
        </div>
      </section>`;
};
